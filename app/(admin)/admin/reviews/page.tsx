...
export default async function AdminReviewsPage() {
  ...
  const { data: pending } = await admin
    ...
    .select(
      "id, user_id, day_key, slot, status, submitted_at, profiles:profiles!validation_submissions_user_id_fkey(full_name,email)",
    )
    .eq("status", "pending")
  ...
  const queue = Array.from(grouped.values()).sort((a, b) =>
        ...
        {queue.map((q) => {
          ...
          return (
            ...
            >
              ...
              <span className="chip">
                {q.count} screenshot{q.count === 1 ? "" : "s"}
              </span>
